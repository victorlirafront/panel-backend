...
export const getServerSidePaths = async (): Promise<{
  paths: { params: MoviePathParams }[];
  fallback: boolean;
}> => {
  try {
    const response = await Axios.get<MovieApiResponse>(FEATURED_API);
    const paths = response.data.results.map(movie => ({
